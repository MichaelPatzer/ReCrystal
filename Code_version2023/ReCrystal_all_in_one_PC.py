...
gain = ['/opt/openmpi-1.4-icc/bin/mpirun -np ',Nproc,' /usr/users/patzer/CRYSTAL/Pcrystal']
trennzeichen = ''
...
gain2 = ['nohup ','/opt/openmpi-1.4-icc/bin/mpirun -np ',Nproc,' /usr/users/patzer/CRYSTAL/Pproperties']
trennzeichen2 = ''
pproperties_command = trennzeichen2.join(gain2)

subprocess.call(str(pproperties_command), shell = True)
subprocess.call("cp INPUT INPUT_hkl", shell = True)
        ...
        for i in range(p,len(tmp)):
            ...
            if 'KEY     XYZ' in tmp[i]:
                ...
                for h in range(i+1, len(tmp)):
                    ...
                    if 'H(' in tmp[h]:
                        t.write(new[0])
                        t.write('  111 000000 0000000000 000000000000000 10 111 11111 0000000 000000000')
                        t.write('\n')
                        continue
...
for i in range(0,len(tmp)):
    ...
    if 'KEY     XYZ' in tmp[i]:
        ...
        for h in range(i+1, len(tmp)):
            ...
            if 'H(' in tmp[h]:
                t.write(new[0])
                t.write('  111 000000 0000000000 000000000000000 00 000 00000 0000000 000000000')
                t.write('\n')
                continue
...
while ref < Cycles:
    ...
    subprocess.call("rm xd* nohup.out", shell = True)
    subprocess.call(str(pcrystal_command), shell = True)
    subprocess.call("rm INPUT", shell = True)
    ...
    subprocess.call("cp INPUT_hkl INPUT", shell = True)
    subprocess.call(str(pproperties_command), shell = True)
